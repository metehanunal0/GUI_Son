...
@njit
def stretching(image, stretch_max_val = 255, stretch_min_val = 0):
    row = len(image)
    column = len(image[0])
...
def icm(image):
    ...
    scene_radiance_res = radiance_rgb(image)
    scene_radiance_res = sv_stretching(scene_radiance_res)
    scene_radiance_res = radiance_rgb(scene_radiance_res)

    return scene_radiance_res